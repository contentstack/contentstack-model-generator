    ...
    public class ContentstackResponse : IResponse, IDisposable
    {
        ...
        private void CopyHeaderValues(HttpResponseMessage response)
        {
            ...
            foreach (var header in response.Headers)
            {
                ...
                var headerValue = GetFirstHeaderValue(response.Headers, header.Key);
                _headers.Add(header.Key, headerValue);
            }
            ...
            if (response.Content != null)
            {
                foreach (var header in response.Content.Headers)
                {
                    ...
                    {
                        headerNames.Add(header.Key);
                        var headerValue = GetFirstHeaderValue(response.Content.Headers, header.Key);
                        _headers.Add(header.Key, headerValue);
                    }
                }
            }
